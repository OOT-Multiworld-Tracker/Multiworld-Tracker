  ...
  render () {
    return (
      <div className='player'>
        <div className='character_name' onClick={() => (app.local.world = app.worlds[this.props.world])}><span>{this.props.current ? "You" : this.state.worldState.save.player_name}</span> <span style={this.state.worldState.save.rupee_count == this.state.worldState.items.wallet.value ? {color: '#e08231'} : null }><img width='16' src={[GreenRupee, BlueRupee, RedRupee][this.state.worldState.items.wallet.Index()]} />{this.state.worldState.save.rupee_count}</span></div>
